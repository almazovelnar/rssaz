...
class Menu extends \yii\widgets\Menu
{
     ...
     * {@inheritdoc}
     */
    public $linkTemplate = '<a href="{url}">{icon} <span class="site-menu-title">{label}</span></a>';
    /**
     * {@inheritdoc}
     * Styles all labels of items on sidebar by AdminLTE
     */
    public $labelTemplate = '{label} {badge} {submenu}';
    /**
     * {@inheritdoc}
     */
     ...
     * {@inheritdoc}
     */
    public $options = ['class' => 'site-menu', 'data-plugin' => 'menu'];
     ...
     * @since 3.0
     */
    public static $iconClassType = '';
    /**
     * @var string
     */
    public static $iconClassPrefix = 'md-';
    ...
    {
        if ($item['header']) {
            return Html::tag('li', $item['header'], ['class' => 'site-menu-category']);
        }

        $submenu = '';

        if (isset($item['items'])) {
            $submenu       = '<i class="site-menu-arrow"></i>';
            $labelTemplate = '<a ' . ($item['active'] ? 'active' : '') . '" href="{url}">{icon} {label}</a>';
            $linkTemplate  = '<a href="{url}">{icon} <span class="site-menu-title">{label}</span></a>';
        } else {
        ...
        $replacements = [
            ...
            '{icon}'    => empty($item['icon']) ? $this->defaultIconHtml
                : '<i class="site-menu-icon ' . (isset($item['iconType']) ? $item['iconType'] : static::$iconClassType) . ' ' . static::$iconClassPrefix . $item['icon'] . '"></i> ',
    ...
    protected function renderItems($items)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            $class   = $item['header'] ? ['nav-header'] : ['site-menu-item'];
            if ($item['active']) {
